...
Ensenso::Ensenso(EnsensoConfig config): min_depth(config.min_depth), max_depth(config.max_depth) {
  ...
  try {
    ...
    factor_depth = 1.0 / (min_depth - max_depth) * (255 * 255 - 1);
...
void Ensenso::configureCapture(EnsensoConfig config) {
  ...
  root[itmParameters][itmRenderPointMap][itmSize][0] = size.width; // [px]
  root[itmParameters][itmRenderPointMap][itmSize][1] = size.height; // [px]
...
cv::Mat Ensenso::takeDepthImage() {
  try {
    ...
    // TODO: Restart Ensenso?

    return cv::Mat(size, CV_16UC1);
  }
}
...
std::pair<cv::Mat, cv::Mat> Ensenso::takeImages() {
  try {
    ...
    return std::make_pair(cv::Mat(size, CV_16UC1), cv::Mat(size, CV_16UC1));
  }
}
